...
# # command frame assistance

# CMD_STARTED     = 1
# CMD_ACK_WAIT    = 2
# CMD_ACKED       = 3
# CMD_ACKED_LATE  = 4
# CMD_ACKED_TOO_LATE = 5
# CMD_ERROR       = 128   # error states are >= 128
# CMD_BAD_SIZE    = 129   # length of cmd frame is less or more than 5 bytes
# CMD_BAD_CHSUM   = 130   # bad checksum of command frame
# CMD_NO_RESPONSE = 131   # no response from any device (for more than 100 ms)
# CMD_BAD_ACK     = 132   # expected 'A' or 'N' but received another value

# class SIOCommand:
#     def __init__(self):
#         # self.lock = threading.Lock()
#         self.state = CMD_STARTED
#         self.data = bytearray()
#         self.ack = 0        # 'A' or 'N'
#         self.sof = timer()  # start of command frame timestamp
#         self.eof = 0.0      # end of command frame timestamp
#         self.toa = 0.0      # time of acknowledgment (ACK/NAK/other received)
#         self.crosstalk = False # whether crosstalk was detected during command frame
#         self.pre_ack = False

#     def __str__(self):
#         return "CF: " + " ".join(["{:02X}".format(b) for b in self.data])

#     def get_stats_str(self):
#         return "CF state={}{}{}".format(
#             self.state, ' X' if self.crosstalk else '',
#             ' ACK +{:.0f}'.format((self.toa - self.eof)*1.e6) if self.toa else ' ',
#             ' {:02X}'.format(self.ack) if self.toa else ' ')
...
#         else:
#             debug_print("Command frame size over limit")

#     def chsum(self):
#         chs = 0
#         for b in self.data[:4]:
#             chs += b
#             if chs >= 256: chs -= 255
#         return chs

#     # def update(self, msg, device=None):
#     #     with self.lock:
#     #         return self._update(msg, device)
        

#     # def _update(self, msg, device=None):
#     def update(self, msg, device=None):
#         """update command state based on arrived message
#         return tuple (True, new state) if new state is final one (command instance will be deleted)
#         return tuple (False, new state) if more processing is expected
#         """
#         prev_state = self.state
#         final = False

#         if self.state == CMD_STARTED:
#             if msg.id == NETSIO_COMMAND_OFF:
#                 # end of command frame
#                 self.eof = timer()
#                 if len(self.data) != 5:
#                     final, self.state = True, CMD_BAD_SIZE
#                 elif self.chsum() != self.data[4]:
#                     final, self.state = True, CMD_BAD_CHSUM
#                 else:
#                     self.state = CMD_ACK_WAIT
#             elif msg.id == NETSIO_DATA_BYTE:
#                 if not device:
#                     # command frame data
#                     self.extend(msg.arg)
#                 else:
#                     # byte from device while command is asserted
#                     self.crosstalk = True
#             elif msg.id  == NETSIO_TRANSMITT_BUFFER and device:
#                 # data from device while command is asserted
#                 self.crosstalk = True

#         elif self.state == CMD_ACK_WAIT:
#             if msg.id == NETSIO_DATA_BYTE:
#                 self.ack = msg.arg[0]
#                 self.toa = timer()
#                 td = self.toa - self.eof
#                 if td > 0.1: # 100 ms
#                     final, self.state = True, CMD_NO_RESPONSE
#                 elif self.ack not in b'AN':
#                     final, self.state = True, CMD_BAD_ACK
#                 else:
#                     if self.ack == 'N':
#                         final = True
#                     if td > 0.020: # 20 ms
#                         self.state = CMD_ACKED_TOO_LATE
#                     elif td > 0.012: # 12 ms, limit is 16 ms, we need some time to deliver ACK to the host
#                         self.state = CMD_ACKED_LATE
#                     else:
#                         self.state = CMD_ACKED

#         #debug_print(prev_state != self.state, final, self.state)
#         return prev_state != self.state, final, self.state


# class ACKEntry:
#     def __init__(self, dev=0, cmd=0, ack=0):
#         self.dev = dev
#         self.cmd = cmd
#         self.ack = ack
#         self.time = timer()
...
# class ACKStore:
#     def __init__(self):
#         self.cache = {} # maps tuple (DEV, CMD) to ACKEntry
#         self.last_cleanup = timer()

#     def get_entry(self, cmd:SIOCommand, default=None):
#         if self.last_cleanup + 1. < timer(): # cleanup expired entries once per second
#             self.cleanup()
#         e = self.cache.get(cmd.data[0] << 8 | cmd.data[1], default)
#         if e:
#             e.time = timer() # update timestamp
#         return e

#     def add_entry(self, cmd:SIOCommand, ack):
#         debug_print("adding entry: {:02X} {:02X} : {:02X}".format(cmd.data[0], cmd.data[1], ack))
#         self.cache[cmd.data[0] << 8 | cmd.data[1]] = ACKEntry(cmd.data[0], cmd.data[1], ack)
...
#             e = self.cache[k] 
#             if e.time + 5. < now:
#                 debug_print("removing entry: {:02X} {:02X} : {:02X}".format(e.dev, e.cmd, e.ack))
#                 self.cache.pop(k)
...
class NetSIOHub:
    ...
    def handle_host_msg(self, msg:NetSIOMsg):
        ...
        #         if self.command:
        #             # print previous command stats
        #             debug_print("-" + self.command.get_stats_str())
        #         # create new command
        #         debug_print("+CF")
        #         self.command = SIOCommand()
        #     elif self.command:
        #         # update existing command
        #         changed, final, state = self.command.update(msg)
        #         # TODO
        #         if changed:
        #             debug_print("*" + self.command.get_stats_str())
        #             if state == CMD_ACK_WAIT:
        #                 debug_print(self.command)
        #                 # get ack entry, if any
        #                 ae = self.ack_store.get_entry(self.command)
        #                 if ae:
        #                     debug_print("pre-ack {:02X}".format(ae.ack))
        #                     self.command.pre_ack = True
        #                     # #self.host_queue.put(NetSIOMsg(NETSIO_DATA_BYTE, ae.ack))
        #             elif final:
        #                 debug_print("-CF")
        #                 self.command = None
    ...
    def handle_device_msg(self, msg:NetSIOMsg, device:NetSIOClient):
        ...
        #     if self.command:
        #         # update existing command
        #         changed, final, state = self.command.update(msg, device)
        #         # TODO
        #         if changed:
        #             debug_print("*" + self.command.get_stats_str())
        #             if state in (CMD_ACKED, CMD_ACKED_LATE, CMD_ACKED_TOO_LATE):
        #                 ae = self.ack_store.get_entry(self.command)
        #                 if not ae:
        #                     # store ack entry
        #                     self.ack_store.add_entry(self.command, msg.arg[0])
        #                 else:
        #                     if self.command.pre_ack:
        #                         # ACK/NAK was already sent
        #                         skip = True
        #                         debug_print("skip, pre-ack'ed")
        #                         if msg.arg[0] != ae.ack:
        #                             debug_print("used wrong pre-ack {:02X} vs {:02X}".format(ae.ack, msg.arg[0]))
        #                             # update ack entry
        #                             self.ack_store.add_entry(self.command, msg.arg[0])
        #                             # TODO send E ?
        #             elif final:
